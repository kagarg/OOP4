...
double EmpiricalDistribution::excess() const {
	double sum = 0;
	double exp_val = expected_value();
	for (int i = 0; i < size; ++i) {
...
std::vector<std::pair<double, double>> EmpiricalDistribution::generate_graph_selection(const std::vector<double>& selection, const int n) const {

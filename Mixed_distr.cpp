...
public:
	MixtureDistribution(Distribution1& d1, Distribution2& d2, double p) : d1(d1), d2(d2), p(p) {}; //конструктор
...
template<class Dist1, class Dist2>
double MixtureDistribution<Dist1, Dist2> ::excess() const {
	return ((1 - p) * (pow((d1.expected_value() - expected_value()), 4) + 6 * d1.dispersion() * pow((d1.expected_value() - expected_value()), 2) +
		4 * (d1.expected_value() - expected_value()) * pow(d1.dispersion(), 3 / 2) * d1.asymmetry() + pow(d1.dispersion(), 2) * d1.excess()) +
		p * (pow((d2.expected_value() - expected_value()), 4) + 6 * d2.dispersion() * pow((d2.expected_value() - expected_value()), 2) +
			4 * (d2.expected_value() - expected_value()) * pow(d2.dispersion(), 3 / 2) * d2.asymmetry() + pow(d2.dispersion(), 2) * d2.excess()) - 3) /
		pow(dispersion(), 2);
}
...
template<class Dist1, class Dist2>
std::vector<std::pair<double, double>> MixtureDistribution<Dist1, Dist2>::generate_graph_selection(const std::vector<double>& selection, const int n) const {
